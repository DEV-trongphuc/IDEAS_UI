...
.profile-book-flipping-leaf .face {
    position: absolute;
    inset: 0;
    background-size: 990px 700px;
    background-repeat: no-repeat;
...
.profile-book-flipping-leaf .face.front {
    background-position: right center;
    border-radius: 0 12px 12px 0;
    border: 1px solid rgba(0, 0, 0, 0.08);
    border-left: none;
...
.profile-book-flipping-leaf .face.front::after {
    content: '';
    position: absolute;
    inset: 0;
    background: linear-gradient(to left, rgba(0,0,0,0.15), rgba(0,0,0,0) 50%);
    pointer-events: none;
}
.profile-book-flipping-leaf .face.back {
    background-position: left center;
...
.profile-book-flipping-leaf .face.back::after {
    content: '';
    position: absolute;
    inset: 0;
    background: linear-gradient(to right, rgba(0,0,0,0.15), rgba(0,0,0,0) 50%);
    pointer-events: none;
}

.profile-book-flipping-leaf.flip-forward-anim {
    animation: profileFlipForward 0.9s cubic-bezier(0.37, 0, 0.63, 1) forwards;
}
.profile-book-flipping-leaf.flip-backward-anim {
    animation: profileFlipBackward 0.9s cubic-bezier(0.37, 0, 0.63, 1) forwards;
}

@keyframes profileFlipForward {
    0% {
        transform: rotateY(0deg) skewY(0deg) scale(1);
    }
    40% {
        transform: rotateY(-75deg) skewY(-3deg) scale(0.98);
    }
    70% {
        transform: rotateY(-135deg) skewY(3deg) scale(0.98);
    }
    100% {
...
@keyframes profileFlipBackward {
    0% {
        transform: rotateY(-180deg) skewY(0deg) scale(1);
    }
    40% {
        transform: rotateY(-105deg) skewY(3deg) scale(0.98);
    }
    70% {
        transform: rotateY(-45deg) skewY(-3deg) scale(0.98);
    }
    100% {
        transform: rotateY(0deg) skewY(0deg) scale(1);
    }
...
@media (max-width: 1100px) {
    ...
    .profile-book-flipping-leaf,
    .profile-book-flipping-leaf .face {
        background-size: calc(85vw) calc(85vw / 1.414) !important;
    }
    .profile-modal-content {
        width: 95vw;
        height: 75vh;
...
function updatePageStates() {
    const leftPanel = document.getElementById('profile-book-left');
    const rightPanel = document.getElementById('profile-book-right');
    const leaf = document.getElementById('profile-book-leaf');
    
    const currentImg = profileImages[currentPage];
    leftPanel.style.backgroundImage = `url('${currentImg}')`;
    rightPanel.style.backgroundImage = `url('${currentImg}')`;
    
    leaf.style.display = 'none';
    leaf.className = 'profile-book-flipping-leaf';
...
function nextProfilePage() {
    ...
    const rightPanel = document.getElementById('profile-book-right');
    const leaf = document.getElementById('profile-book-leaf');
    const leafFront = document.getElementById('profile-book-leaf-front');
    const leafBack = document.getElementById('profile-book-leaf-back');
    
    const nextImg = profileImages[currentPage + 1];
    const currentImg = profileImages[currentPage];
    
    leafFront.style.backgroundImage = `url('${currentImg}')`;
    leafBack.style.backgroundImage = `url('${nextImg}')`;
    
    rightPanel.style.backgroundImage = `url('${nextImg}')`;
    
    leaf.style.display = 'block';
    leaf.className = 'profile-book-flipping-leaf flip-forward-anim';
...
function prevProfilePage() {
    ...
    const prevImg = profileImages[currentPage - 1];
    const currentImg = profileImages[currentPage];
    
    leafFront.style.backgroundImage = `url('${prevImg}')`;
    leafBack.style.backgroundImage = `url('${currentImg}')`;
    
    leftPanel.style.backgroundImage = `url('${prevImg}')`;
    
    leaf.style.display = 'block';
    leaf.className = 'profile-book-flipping-leaf flip-backward-anim';
