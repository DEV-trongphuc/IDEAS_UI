...
if (grid.dataset.sliderId) return;
...
function initHeroBadgeTyping(){const badgeSpan=document.querySelector('.hero-badge span:last-of-type');if(badgeSpan){const originalText=badgeSpan.textContent.trim();if(originalText.toLowerCase().includes('tri thức nguyên bản')){badgeSpan.textContent='';let charIndex=0;const typingSpeed=80;function type(){if(charIndex<originalText.length){badgeSpan.textContent+=originalText.charAt(charIndex);charIndex++;setTimeout(type,typingSpeed)}}type()}}}
function initialize() {
initBackToTop();
initMobileDropdowns();
initMobileSliders();
...
if (!backSrc) {
backSrc = 'https://ideas.edu.vn/wp-content/uploads/2025/11/Sample_page-0002.webp';
