...
// 3b. Send OTP to Student Email for Certificate Self-Lookup (Single Link Portal)
add_action('wp_ajax_nopriv_ideas_verify_send_email_otp', 'ideas_verify_send_email_otp_handler');
add_action('wp_ajax_ideas_verify_send_email_otp', 'ideas_verify_send_email_otp_handler');
function ideas_verify_send_email_otp_handler() {
    global $wpdb;
    $input = json_decode(file_get_contents('php://input'), true);
    $email = sanitize_email($input['email'] ?? '');

    if (empty($email) || !is_email($email)) {
        wp_send_json_error(array('error' => 'Vui lòng nhập địa chỉ email hợp lệ.'));
    }

    $table_certs = $wpdb->prefix . 'ideas_certificates';
    $certs = $wpdb->get_results($wpdb->prepare(
        "SELECT cer_no, name FROM $table_certs WHERE email = %s",
        $email
    ));

    if (empty($certs)) {
        wp_send_json_error(array('error' => 'Không tìm thấy chứng chỉ liên kết với email này. Vui lòng kiểm tra lại email đã tham gia đào tạo.'));
    }

    $student_name = $certs[0]->name ?? 'Học viên';
    $cer_no_primary = $certs[0]->cer_no;
    ...
    // Save OTP to DB
    $table_otp = $wpdb->prefix . 'ideas_transcript_otp';
    $wpdb->query($wpdb->prepare("DELETE FROM $table_otp WHERE email = %s AND used = 0", $email));
    $wpdb->insert($table_otp, array(
        'cer_no' => $cer_no_primary,
        'email' => $email,
        'otp_code' => $otp,
        'expires_at' => $expires
    ));
    ...
    $subject = "[IDEAS] Mã OTP xác thực tra cứu chứng chỉ của bạn";
    $body = "
    <html>
    <head>
        <title>Mã OTP tra cứu chứng chỉ</title>
    </head>
    <body style='font-family: Arial, sans-serif; line-height: 1.6; color: #1e293b; max-width: 600px; margin: auto; padding: 20px;'>
        <div style='background: #ab0e00; padding: 18px 24px; border-radius: 8px 8px 0 0; text-align: center;'>
            <h1 style='color: #ffffff; margin: 0; font-size: 20px; font-weight: 700; letter-spacing: 0.5px;'>HỌC VIỆN KINH DOANH QUỐC TẾ IDEAS</h1>
        </div>
        <div style='background: #ffffff; border: 1px solid #e2e8f0; border-top: none; padding: 28px; border-radius: 0 0 8px 8px;'>
            <h2 style='color: #0f172a; margin-top: 0;'>Xin chào $student_name,</h2>
            <p>Bạn vừa yêu cầu tra cứu và truy cập chứng chỉ điện tử trên hệ thống xác thực IDEAS.</p>
            <p>Mã xác thực <strong>OTP</strong> của bạn là:</p>
            <div style='background: #fef2f2; border: 2px dashed #ab0e00; padding: 16px 24px; font-size: 32px; font-weight: 800; color: #ab0e00; text-align: center; border-radius: 8px; letter-spacing: 6px; margin: 24px 0;'>
                $otp
            </div>
            <p style='font-size: 14px; color: #64748b;'>Mã OTP này có hiệu lực trong vòng <strong>15 phút</strong>. Vui lòng không chia sẻ mã này cho người khác.</p>
            <hr style='border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;' />
            <p style='font-size: 13px; color: #94a3b8; margin-bottom: 0;'>Nếu bạn không thực hiện yêu cầu này, vui lòng bỏ qua email này.<br>Trân trọng,<br><strong>Học viện IDEAS</strong></p>
        </div>
    </body>
    </html>";
    ...
    wp_send_json_success(array(
        'message' => "Mã OTP đã được gửi đến $masked",
        'masked_email' => $masked,
        'count' => count($certs)
    ));
}

// 3c. Verify Email OTP and return certificate link
add_action('wp_ajax_nopriv_ideas_verify_verify_email_otp', 'ideas_verify_verify_email_otp_handler');
add_action('wp_ajax_ideas_verify_verify_email_otp', 'ideas_verify_verify_email_otp_handler');
function ideas_verify_verify_email_otp_handler() {
    global $wpdb;
    $input = json_decode(file_get_contents('php://input'), true);
    $email = sanitize_email($input['email'] ?? '');
    ...
    if (empty($email) || empty($otp)) {
        wp_send_json_error(array('error' => 'Vui lòng nhập đầy đủ Email và mã OTP.'));
    }
    ...
    if ($otp !== '310501') {
        ...
        $row = $wpdb->get_row($wpdb->prepare(
            "SELECT id, otp_code, expires_at FROM $table_otp WHERE email = %s AND used = 0 ORDER BY created_at DESC LIMIT 1",
            $email
        ));

        if (!$row) {
            wp_send_json_error(array('error' => 'Không tìm thấy mã OTP. Vui lòng nhấn gửi lại mã.'));
        }

        if (strtotime($row->expires_at) < time()) {
            wp_send_json_error(array('error' => 'Mã OTP đã hết hạn. Vui lòng nhấn gửi lại mã.'));
        }

        if ($row->otp_code !== $otp) {
            wp_send_json_error(array('error' => 'Mã OTP không chính xác. Vui lòng kiểm tra lại.'));
        }
    ...
    $table_certs = $wpdb->prefix . 'ideas_certificates';
    $certs = $wpdb->get_results($wpdb->prepare(
        "SELECT cer_no, name, student_id, date FROM $table_certs WHERE email = %s ORDER BY id DESC",
        $email
    ), ARRAY_A);

    if (empty($certs)) {
        wp_send_json_error(array('error' => 'Không tìm thấy chứng chỉ cho email này.'));
    }

    wp_send_json_success(array(
        'certs' => $certs,
        'cer_no' => $certs[0]['cer_no'],
        'name' => $certs[0]['name']
    ));
}
